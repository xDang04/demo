...
class DispatcherTest extends \PHPUnit\Framework\TestCase {
    ...
    public function testReverseRouteWithDashes()
    {
        ...
        $r->any( array('course-catalogue/store/{store:i}?', 'products'), array(__NAMESPACE__.'\\Test','route'));

        $this->assertEquals('course-catalogue/store', $r->route('products'));
        $this->assertEquals('course-catalogue/store/1', $r->route('products', array(1)));
    }
    ...
    public function testPrefixGroupsReverseRoutes()
    {
        ...
        $r->group(['prefix' => 'course-catalogue/store'], function($r) {
            $r->any(['items/{store:i}?', 'products'], array(__NAMESPACE__.'\\Test','route'));
        });

        $this->assertEquals('course-catalogue/store/items', $r->route('products'));
        $this->assertEquals('course-catalogue/store/items/1', $r->route('products', array(1)));
    }
